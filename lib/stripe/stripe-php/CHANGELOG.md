...
* [#1903](https://github.com/stripe/stripe-php/pull/1903) ⚠️ Add strongly typed EventNotifications
  ...
  - ⚠️ removed the `Util::json_decode_thin_event_object`. Its functionality was folded into the new `\PublishPress\Stripe\V2\EventNotification::fromJson` method.
...
* [#1673](https://github.com/stripe/stripe-php/pull/1673)
  ...
  * This release changes the pinned API version to `2024-04-10`. Please read the [API Changelog](https://docs.stripe.com/changelog/2024-04-10) and carefully review the API changes before upgrading.
  ...
  ```php
  $curl = new \PublishPress\Stripe\HttpClient\CurlClient([
    CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4
  ]);
  \PublishPress\Stripe\ApiRequestor::setHttpClient($curl);
  ```
...
* [#1647](https://github.com/stripe/stripe-php/pull/1647) Fix \PublishPress\Stripe\Tax\Settings::update
...
* [#1612](https://github.com/stripe/stripe-php/pull/1612) Report usage of .save and StripeClient
  * Reports uses of the deprecated `.save` and of `StripeClient` in `X-Stripe-Client-Telemetry`. (You can disable telemetry via `\PublishPress\Stripe\Stripe::setEnableTelemetry(false);`, see the [README](https://github.com/stripe/stripe-php/blob/master/README.md#telemetry).)
...
2. **(Alternative option) Specify a version other than `2023-08-16` when initializing `stripe-php`.**
     ...
     - $stripe = new \PublishPress\Stripe\StripeClient('STRIPE_KEY_REDACTED');
     + $stripe = new \PublishPress\Stripe\StripeClient([
     +   'api_key' => 'STRIPE_KEY_REDACTED',
         'stripe_version' => '2020-08-27',
     + ]);

       // if using the global client
       Stripe.apiKey = "STRIPE_KEY_REDACTED";
...
* [#1241](https://github.com/stripe/stripe-php/pull/1241) Add generic parameter to \PublishPress\Stripe\Collection usages
...
* [#1155](https://github.com/stripe/stripe-php/pull/1155) Add streaming methods to Service infra
  ...
  * `\PublishPress\Stripe\RequestOptions::parse` now clones its input if it is already a `RequestOptions` object, to prevent accidental mutation.
...
## 7.25.0 - 2020-02-14
* [#879](https://github.com/stripe/stripe-php/pull/879) Make `\PublishPress\Stripe\Collection` implement `\Countable`
...
-   Use `\PublishPress\Stripe\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\PublishPress\Stripe\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\PublishPress\Stripe\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\PublishPress\Stripe\Source`
...
-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\PublishPress\Stripe\FileUpload` to `\PublishPress\Stripe\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\PublishPress\Stripe\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\PublishPress\Stripe\Sigma\ScheduledQueryRun`) for Sigma
...
-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\PublishPress\Stripe\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\PublishPress\Stripe\Invoice`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\PublishPress\Stripe\Discount` and `\PublishPress\Stripe\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\PublishPress\Stripe\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\PublishPress\Stripe\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\PublishPress\Stripe\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\PublishPress\Stripe\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\PublishPress\Stripe\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\PublishPress\Stripe\Event` class
...
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\PublishPress\Stripe\Error\SignatureVerification`
...
*   `\PublishPress\Stripe\AttachedObject` no longer exists. Attributes that used to be instances of `\PublishPress\Stripe\AttachedObject` (such as `metadata`) are now instances of `\PublishPress\Stripe\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\PublishPress\Stripe\Account` instances) are now instances of `\PublishPress\Stripe\StripeObject`, except when they are empty. `\PublishPress\Stripe\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\PublishPress\Stripe\Collection` now derives from `\PublishPress\Stripe\StripeObject` rather than from `\PublishPress\Stripe\ApiResource`.
...
-   Add `\PublishPress\Stripe\Stripe::setLogger()` to support an external PSR-3 compatible logger
...
-   Rename `\PublishPress\Stripe\Object` to `\PublishPress\Stripe\StripeObject` (PHP 7 compatibility)
