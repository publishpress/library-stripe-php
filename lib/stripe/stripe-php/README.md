...
$curl->setTimeout(10); // default is \PublishPress\Stripe\HttpClient\CurlClient::DEFAULT_TIMEOUT
$curl->setConnectTimeout(5); // default is \PublishPress\Stripe\HttpClient\CurlClient::DEFAULT_CONNECT_TIMEOUT
...
```php
// set up your tweaked Curl client
$curl = new \PublishPress\Stripe\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
\PublishPress\Stripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\PublishPress\Stripe\Stripe::setLogger($logger);
```
...
```php
$curl = new \PublishPress\Stripe\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
\PublishPress\Stripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\PublishPress\Stripe\Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
\PublishPress\Stripe\Stripe::setMaxNetworkRetries(2);
```
...
```php
\PublishPress\Stripe\Stripe::setEnableTelemetry(false);
```
...
// $response->body is a string, you can call $stripe->deserialize to get a \PublishPress\Stripe\StripeObject.
...
```php
\PublishPress\Stripe\Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
